...
/*   Updated: 2025/11/16 16:49:03 by mlaktaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*storage_list[1024];
	char		*result;

	if (fd < 0 || fd >= 1024 || BUFFER_SIZE <= 0)
		return (NULL);
	if (!read_and_append(fd, &storage_list[fd]))
		return (NULL);
	if (!storage_list[fd] || storage_list[fd][0] == '\0')
		return (NULL);
	result = extract_line(storage_list[fd]);
	if (!result)
		return (free(storage_list[fd]), storage_list[fd] = NULL, NULL);
	if (!clean_storage(&storage_list[fd]))
